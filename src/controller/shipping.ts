...
export const createShipping = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const messenger = await User.findOne({ role: 'MESSENGER' }).select('firstname lastname');
        if (!messenger) {
            res.status(404).json({ success: false, message: 'No messenger user found' });
            return ;
        }
        const tracking_number_data = await DailyOrderList.findOne({ tracking_number: tracking_number }).select('_id');
        if (!tracking_number_data) {
            res.status(404).json({ success: false, message: 'Not found an _id that match tracking number ' });
            return ;
        }
        const customerAddress = await User.findOne({daily_order_list : tracking_number_data}).select('addressName');
        if (!customerAddress) {
            res.status(404).json({ success: false, message: 'Can not get customer address that has a matching tracking number' });
            return ;
        }
        ...
        res.status(500).json({ success: false, message: 'Failed to retrieve shipping data', error: error.message });
    }
};
...
export const updateShipping = async (req: Request, res: Response): Promise<void> => {
    try {
        ...
        const allowedStatuses = ['Ongoing', 'Derivered', 'Returned', 'Failed to Deliver'];
...
// for test
export const deleteShippingById = async (req: Request, res: Response): Promise<void> => {
    try {
        const { id } = req.params;
        ...
        res.status(200).json({ message: 'Shipping deleted successfully', shipping: deletedShipping });
    } catch (error: any) {
